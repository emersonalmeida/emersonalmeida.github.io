...
# -*- coding: utf-8 -*-
"""
Mini Research v5.0 - Coletor de Dados Multi-Fonte com Análises e Dashboard
                ...
                if opcao == 1:
                    try:
                        ...
                        if r and "top" in r and r["top"] is not None:
                            ...
                            print_progress(f"Top relacionados ({tipo_nome}, {periodo_nome}): {len(df)} itens")
                    ...
                    try:
                        ...
                        if not regioes.empty:
                            ...
                            print_progress(f"Interesse por regiões ({tipo_nome}, {periodo_nome}): {len(serie)} itens")
...
def run_serp(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> List[Dict]:
    ...
    for buscador_idx, bus_id in enumerate(buscadores_order, 1):
        ...
        print_progress(f"Buscando em {nome} ({buscador_idx}/{len(buscadores_order)})...")
...
def run_youtube(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> Dict:
    ...
    for order_idx, order in enumerate(orders_order, 1):
        ...
        if videos:
            ...
            print_progress(f"  {order_name}: {len(videos)} vídeos encontrados")
    ...
    for loja_idx, loja_id in enumerate(lojas_order, 1):
        if loja_id == 1:
            ...
            if not df_google.empty:
                ...
                print_progress(f"Google Play: {len(df_google)} apps encontrados (solicitados: {n_apps_efetivo})")
...
def coletar_configuracao() -> Optional[Dict]:
    ...
    print(f"\n{cyan(bold('Mini Research v5.0 - Configuração'))}\n")
...
def main():
    ...
    print(f"\n{cyan(bold('Mini Research v5.0 - Coletor de Dados Multi-Fonte'))}\n")
