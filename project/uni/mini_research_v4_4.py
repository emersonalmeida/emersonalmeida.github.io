...
# -*- coding: utf-8 -*-
"""
Mini Research v4.4 - Coletor de Dados Multi-Fonte com Análises e Dashboard
Versão otimizada com melhorias de código, performance e funcionalidades

Melhorias v4.4:
- Eliminação de código repetitivo com funções auxiliares reutilizáveis
- Melhor tratamento de erros com logging e retry automático
- Validação e sanitização completa de inputs
- Suporte completo a múltiplos termos de busca
- Barras de progresso com tqdm para todas as operações
- Função genérica para execução de fontes (elimina repetição)
- Type hints e docstrings melhoradas
- Validação de API keys antes de usar
- Delay adaptativo baseado em tempo de resposta
- Refatoração de consolidar_dados para reduzir repetição
...
def run_suggest(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> List[Dict]:
    ...
    if resultados:
        ...
        logger.info(f"Google Suggest: {len(resultados)} itens coletados")
...
def consolidar_dados(resultados: Dict, termo: str, output_dir: str, fontes_ordenadas: Optional[List[int]] = None) -> pd.DataFrame:
    """Consolida todos os dados coletados em um único DataFrame - v4.4 refatorado"""
...
def coletar_configuracao() -> Optional[Dict]:
    """Coleta configuração otimizada v4.4"""
    print("\nMini Research v4.4 - Configuração\n")
...
def main():
    """Função principal otimizada v4.4"""
    print("\nMini Research v4.4 - Coletor de Dados Multi-Fonte\n")
    ...
    df_consolidado = consolidar_dados(resultados, termo_principal, output_dir, config.get("fontes_ordenadas"))
