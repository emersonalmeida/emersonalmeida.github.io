...
# -*- coding: utf-8 -*-
"""
Mini Research v2 - Coletor de Dados Multi-Fonte
Versão completamente padronizada com foco em UX/UI/CLI

Melhorias v2:
- Menus 100% padronizados (sempre numéricos, múltiplas seleções)
- Ordem lógica: termo → região → fontes → configurações → parâmetros
- Feedback constante do que está sendo configurado
- Chaves API preenchidas corretamente
- Configurações globais para lojas
- Melhores práticas UX/UI/CLI aplicadas
"""
...
def parse_numeric_input(input_str, options_dict, default=None, allow_multiple=True):
    """
...
def print_menu(title, options, default=None, show_all_option=True):
    """
    Menu padronizado - SEMPRE numérico
    Formato: 1. Opção, 2. Opção, ..., t. Todos
    """
    print(cyan(f"\n{title}"))
    print("-" * 70)
    
    for key, value in sorted(options.items()):
        if key == "t" and show_all_option:
            marker = green("t")
            print(f"  {marker}. {value} (seleciona todas as opções acima)")
    ...
    if show_all_option:
        ...
        print(gray("  💡 Dica: Digite números separados por vírgula (ex: 1,2,3)"))
...
def print_config_summary(config):
    ...
    print(f"  Regiões: {', '.join(config.get('regions', []))}")
    print(f"  Fontes selecionadas: {len(config.get('fontes', []))} fonte(s)")
    print(f"  Delay: {config.get('delay', 1.0)}s")
    print("-" * 70)
...
# ======================================
# 🎯 CONFIGURAÇÃO (Ordem Lógica)
# ======================================

def coletar_configuracao():
    """Coleta configuração em ordem lógica: termo → região → fontes → configurações → parâmetros"""
    print_header("CONFIGURAÇÃO DE COLETA")
    print(cyan("Siga a ordem lógica para configurar sua pesquisa\n"))
    
    config = {}
    
    # 1. TERMO (obrigatório)
    print(cyan("1️⃣  TERMO DE BUSCA"))
    print("-" * 70)
    ...
    # 2. REGIÃO (global)
    print(cyan("2️⃣  REGIÃO"))
    print("-" * 70)
    print_menu("Selecione as regiões:", REGIONS_OPTIONS, default=1)
    ...
    # 3. FONTES DE DADOS
    print(cyan("3️⃣  FONTES DE DADOS"))
    print("-" * 70)
    print_menu("Selecione as fontes de dados:", FONTES_OPTIONS, default=None)
    ...
    print(cyan("4️⃣  PARÂMETROS GLOBAIS"))
    print("-" * 70)
    ...
    # 5. CONFIGURAÇÕES ESPECÍFICAS POR FONTE
    print(cyan("5️⃣  CONFIGURAÇÕES ESPECÍFICAS POR FONTE"))
    print("-" * 70)
    print(gray("Configure apenas as fontes que você selecionou acima\n"))
    
    # Google Suggest
    if 1 in config["fontes"]:
        print(cyan("\n--- Google Suggest ---"))
        print_menu("Clientes:", CLIENTS_OPTIONS, default=1)
        clients_input = input("> Selecione clientes [1]: ").strip() or "1"
        config["suggest"] = {
            "clients": parse_numeric_input(clients_input, CLIENTS_OPTIONS, default=1),
            "sources": parse_numeric_input(input("> Fontes (1=web,2=youtube,3=news,4=shopping) [1]: ").strip() or "1", SOURCES_OPTIONS, default=1),
            "opcoes": parse_numeric_input(input("> Opções (1=top,2=a-z,3=0-9,4=categorias) [1]: ").strip() or "1", SUGGEST_OPCOES, default=1),
    ...
    if 2 in config["fontes"]:
        print(cyan("\n--- Google Trends ---"))
        print_menu("Tipos de pesquisa:", TRENDS_TIPOS, default=1)
        tipos_input = input("> Selecione tipos [1]: ").strip() or "1"
        config["trends"] = {
            "gtypes": parse_numeric_input(tipos_input, TRENDS_TIPOS, default=1),
            "timeframe": parse_numeric_input(input("> Período (1-6) [4]: ").strip() or "4", TRENDS_PERIODOS, default=4)[0],
            "opcoes": parse_numeric_input(input("> Opções (1=top,2=rising,3=regiões,4=tempo) [1,2,3,4]: ").strip() or "1,2,3,4", TRENDS_OPCOES, default=1),
    ...
    if 4 in config["fontes"]:
        print(cyan("\n--- YouTube ---"))
        print_menu("Ordenar por:", YOUTUBE_ORDER, default=1)
    ...
    if 5 in config["fontes"]:
        ...
        if config["play_store"]["coletar_reviews"]:
            ...
            if apps_sel.lower() in ["t", "todos"]:
                config["play_store"]["apps_selecionados"] = "todos"
            else:
                config["play_store"]["apps_selecionados"] = [int(x) for x in apps_sel.split(",") if x.strip().isdigit()]
        else:
            config["play_store"]["max_reviews"] = 0
            config["play_store"]["apps_selecionados"] = [1, 2, 3]
        print(green("✓ Google Play Store configurado\n"))
    ...
    if 6 in config["fontes"]:
        ...
        if config["app_store"]["coletar_reviews"]:
            ...
            apps_sel = input("> Quais apps? (1,2,3 ou 't' para todos) [1,2,3]: ").strip() or "1,2,3"
            if apps_sel.lower() in ["t", "todos"]:
                config["app_store"]["apps_selecionados"] = "todos"
            else:
                config["app_store"]["apps_selecionados"] = [int(x) for x in apps_sel.split(",") if x.strip().isdigit()]
        else:
            config["app_store"]["max_reviews"] = 0
            config["app_store"]["apps_selecionados"] = [1, 2, 3]
        print(green("✓ Apple App Store configurado\n"))
...
def main():
    print_header("MINI RESEARCH v2 - COLETOR DE DADOS MULTI-FONTE")
    print(cyan("Versão completamente padronizada com foco em UX/UI/CLI\n"))
