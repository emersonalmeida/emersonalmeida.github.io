...
# -*- coding: utf-8 -*-
"""
Mini Research v4.2 - Coletor de Dados Multi-Fonte com Análises e Dashboard
Versão com menu melhorado e interface aprimorada

Melhorias v4.2:
- Menu redesenhado com melhor hierarquia visual
- Suporte a múltiplos termos separados por vírgula
- Estrutura de menu mais limpa e organizada
- Submenus mais claros e intuitivos
- Valores padrão mais explícitos
- Melhor formatação visual seguindo padrões UX/UI
- Navegadores separados para Suggest
- Ordenação de comentários do YouTube
- Ordenação de reviews das App Stores
...
def print_header(title, description=""):
    """Cabeçalho padronizado com descrição - Versão melhorada v4.2"""
    print(cyan("\n" + "#" * 70))
    print(cyan(f"# {title}"))
    if description:
...
def print_menu(title, description, options, default=None, show_todos=True):
    """Menu padronizado - SEMPRE numérico - Versão melhorada v4.2"""
    print(cyan(f"\n# {title}"))
    if description:
        print(gray(f"  {description}"))
    print("-" * 70)
...
def run_suggest(termo, config, output_dir, resultados_tempo_real):
    ...
    clients_map = {1: "chrome", 2: "firefox", 3: "safari", 4: "chrome"}  # Safari usa chrome como fallback
...
def coletar_configuracao():
    """Coleta configuração em ordem lógica melhorada v4.2: termo(s) → região → fontes → configurações → delay"""
    print_header("MINI RESEARCH v4.2 - COLETOR DE DADOS MULTI-FONTE", "")
    
    config = {}
    
    # 01. TERMOS DE BUSCA (suporte a múltiplos termos separados por vírgula)
    print_header("01. TERMOS DE BUSCA", "Separe por vírgula")
    ...
    # Processar múltiplos termos
    termos = [t.strip() for t in termos_input.split(",") if t.strip()]
    if not termos:
        print(red("❌ Nenhum termo válido encontrado!"))
        return None
    
    # Por enquanto, usar o primeiro termo (pode ser expandido no futuro)
    config["termo"] = termos[0]
    config["termos"] = termos  # Armazenar todos os termos para uso futuro
    print(green(f"✓ Termo configurado: {bold(termos[0])}\n"))
    
    # 02. REGIÃO (global)
    regions_with_todos = {**REGIONS_OPTIONS, 9: "Todos"}
    print_menu("02. REGIÃO", "", regions_with_todos, default=1)
    regions_input = input("# > [1]: ").strip() or "1"
    if regions_input.lower() in ["t", "todos", "9"]:
        regions_selected = list(range(1, 9))
    else:
    ...
    # 03. PLATAFORMAS
    fontes_with_todos = {**FONTES_OPTIONS, 6: "Todos"}
    print_menu("03. PLATAFORMAS", "", fontes_with_todos, default=None)
    fontes_input = input("# > [6]: ").strip() or "6"
    if fontes_input.lower() in ["t", "todos", "6"]:
        fontes = list(range(1, 6))
    else:
    ...
    # 04-08. CONFIGURAÇÕES ESPECÍFICAS POR FONTE (na ordem das fontes selecionadas)
    ...
    for fonte_id in fontes_ordenadas:
        if fonte_id == 1:  # Google Suggest
            print_header("04. SUGGEST", "")
            
            # 04. SUGGEST - NAVEGADORES
            clients_with_todos = {**CLIENTS_OPTIONS, 5: "Todos"}
            print_menu("04. SUGGEST - NAVEGADORES", "", clients_with_todos, default=1)
            clients_input = input("# > [1]: ").strip() or "1"
            if clients_input.lower() in ["t", "todos", "5"]:
                clients_selected = list(range(1, 5))
            else:
                clients_selected = parse_numeric_input(clients_input, CLIENTS_OPTIONS, default=1)
            
            # 04. SUGGEST - FONTES
            sources_with_todos = {**SOURCES_OPTIONS, 5: "Todos"}
            print_menu("04. SUGGEST - FONTES", "", sources_with_todos, default=1)
            sources_input = input("# > [1,2]: ").strip() or "1,2"
            if sources_input.lower() in ["t", "todos", "5"]:
                sources_selected = list(range(1, 5))
            else:
                sources_selected = parse_numeric_input(sources_input, SOURCES_OPTIONS, default=1)
            
            # 04. SUGGEST - DADOS
            suggest_opcoes_with_todos = {**SUGGEST_OPCOES, 5: "Todos"}
            print_menu("04. SUGGEST - DADOS", "", suggest_opcoes_with_todos, default=1)
            opcoes_input = input("# > [5]: ").strip() or "5"
            if opcoes_input.lower() in ["t", "todos", "5"]:
                opcoes_selected = list(range(1, 5))
            else:
                opcoes_selected = parse_numeric_input(opcoes_input, SUGGEST_OPCOES, default=1)
            
            # 04. SUGGEST - RESULTADOS
            print(cyan("\n# 04. SUGGEST - RESULTADOS"))
            print("-" * 70)
            limit_input = input("# > [15]: ").strip() or "15"
            ...
            print_header("05. TRENDS", "")
            
            # 05. TRENDS - FONTES
            trends_tipos_with_todos = {**TRENDS_TIPOS}
            print_menu("05. TRENDS - FONTES", "", trends_tipos_with_todos, default=1)
            tipos_input = input("# > [1,2,3]: ").strip() or "1,2,3"
            tipos_selected = parse_numeric_input(tipos_input, TRENDS_TIPOS, default=1)
            
            # 05. TRENDS - PERÍODOS
            print_menu("05. TRENDS - PERÍODOS", "", TRENDS_PERIODOS, default=4)
            periodo_input = input("# > [1,2,3,4,6]: ").strip() or "1,2,3,4,6"
            periodos_selected = parse_numeric_input(periodo_input, TRENDS_PERIODOS, default=4)
            
            # 05. TRENDS - DADOS
            trends_opcoes_with_todos = {**TRENDS_OPCOES, 5: "Todos"}
            print_menu("05. TRENDS - DADOS", "", trends_opcoes_with_todos, default=1)
            opcoes_input = input("# > [1,2,3,4]: ").strip() or "1,2,3,4"
            if opcoes_input.lower() in ["t", "todos", "5"]:
                opcoes_selected = list(range(1, 5))
            else:
                opcoes_selected = parse_numeric_input(opcoes_input, TRENDS_OPCOES, default=1)
            
            # 05. TRENDS - RESULTADOS
            print(cyan("\n# 05. TRENDS - RESULTADOS"))
            print("-" * 70)
            topn_input = input("# > [20]: ").strip() or "20"
            topn = int(topn_input) if topn_input.isdigit() else 20
            
            config["trends"] = {
                "gtypes": tipos_selected,
                "timeframe": periodos_selected[0] if periodos_selected else 4,
                "opcoes": opcoes_selected,
                "topn": topn,
            }
            ...
            print_header("06. SERP", "")
            
            # 06. SERP - BUSCADORES
            serp_buscadores_with_todos = {**SERP_BUSCADORES, 5: "Todos"}
            print_menu("06. SERP - BUSCADORES", "", serp_buscadores_with_todos, default=1)
            buscadores_input = input("# >  [1,2,3,4]: ").strip() or "1,2,3,4"
            if buscadores_input.lower() in ["t", "todos", "5"]:
                buscadores_selected = list(range(1, 5))
            else:
                buscadores_selected = parse_numeric_input(buscadores_input, SERP_BUSCADORES, default=1)
            
            # 06. SERP - RESULTADOS
            print(cyan("\n# 06. SERP - RESULTADOS"))
            print("-" * 70)
            limite_input = input("# > [20]: ").strip() or "20"
            ...
            print_header("07. YOUTUBE", "")
            
            # 07. YOUTUBE - VIDEOS
            print(cyan("\n# 07. YOUTUBE - VIDEOS"))
            print("-" * 70)
            limite_videos_input = input("# > [10]: ").strip() or "10"
            limite_videos = int(limite_videos_input) if limite_videos_input.isdigit() else 10
            
            # 07. YOUTUBE - VIDEOS - ORDENAÇÃO
            youtube_order_with_todos = {**YOUTUBE_ORDER, 4: "Todos"}
            print_menu("07. YOUTUBE - VIDEOS - ORDENAÇÃO", "", youtube_order_with_todos, default=1)
            order_input = input("# > [1,2,3]: ").strip() or "1,2,3"
            if order_input.lower() in ["t", "todos", "4"]:
                order_selected = list(range(1, 4))
            else:
                order_selected = parse_numeric_input(order_input, YOUTUBE_ORDER, default=1)
            
            # 07. YOUTUBE - COMENTÁRIOS
            print(cyan("\n# 07. YOUTUBE - COMENTÁRIOS"))
            print("-" * 70)
            limite_comentarios_input = input("# > [10]: ").strip() or "10"
            limite_comentarios = int(limite_comentarios_input) if limite_comentarios_input.isdigit() else 10
            
            # 07. YOUTUBE - COMENTÁRIOS - ORDENACAO
            youtube_comment_order_with_todos = {**YOUTUBE_COMMENT_ORDER, 4: "Todos"}
            print_menu("07. YOUTUBE - COMENTÁRIOS - ORDENACAO", "", youtube_comment_order_with_todos, default=1)
            comment_order_input = input("# > [1,2,3]: ").strip() or "1,2,3"
            if comment_order_input.lower() in ["t", "todos", "4"]:
                comment_order_selected = list(range(1, 4))
            else:
                comment_order_selected = parse_numeric_input(comment_order_input, YOUTUBE_COMMENT_ORDER, default=1)
            ...
            config["youtube"] = {
                ...
                "videos_selecionados": "todos",  # Por padrão, todos os vídeos
            }
            print(green("✓ YouTube configurado\n"))
        
        elif fonte_id == 5:  # App Stores
            print_header("08. APP STORE", "")
            
            # 08. APP STORE - LOJAS
            lojas_options = {1: "Google Play Store", 2: "Apple App Store", 3: "Todos"}
            print_menu("08. APP STORE - LOJAS", "", lojas_options, default=3)
            lojas_input = input("# > [1,2]: ").strip() or "1,2"
            if lojas_input.lower() in ["t", "todos", "3"]:
                lojas_selected = [1, 2]
            else:
                lojas_selected = parse_numeric_input(lojas_input, {1: "Google Play Store", 2: "Apple App Store"}, default=1)
            
            # 08. APP STORE - APLICATIVOS
            print(cyan("\n# 08. APP STORE - APLICATIVOS"))
            print("-" * 70)
            n_apps_input = input("# > [10]: ").strip() or "10"
            n_apps = int(n_apps_input) if n_apps_input.isdigit() else 10
            
            # 08. APP STORE - REVIEWS
            print(cyan("\n# 08. APP STORE - REVIEWS"))
            print("-" * 70)
            max_reviews_input = input("# > [100]: ").strip() or "100"
            max_reviews = int(max_reviews_input) if max_reviews_input.isdigit() else 100
            
            # 08. APP STORE - REVIEWS - ORDENAÇÃO
            app_store_review_order_with_todos = {**APP_STORE_REVIEW_ORDER, 4: "Todos"}
            print_menu("08. APP STORE - REVIEWS - ORDENAÇÃO", "", app_store_review_order_with_todos, default=1)
            review_order_input = input("# > [1,2,3]: ").strip() or "1,2,3"
            if review_order_input.lower() in ["t", "todos", "4"]:
                review_order_selected = list(range(1, 4))
            else:
                review_order_selected = parse_numeric_input(review_order_input, APP_STORE_REVIEW_ORDER, default=1)
            ...
            config["stores"] = {
                ...
                "apps_selecionados": "todos",  # Por padrão, todos os apps
            }
    ...
    # DELAY (última pergunta antes de iniciar)
    print(cyan("\n# DELAY"))
    print("-" * 70)
    delay_input = input("# > DELAY [0]: ").strip() or "0"
    ...
    print(cyan("\n# CONFIRMAÇÃO"))
    print("-" * 70)
    confirm_input = input("# > INICIAR COLETA? (S/N) [S]: ").strip().lower() or "s"
    if confirm_input not in ["s", "sim", "y", "yes"]:
