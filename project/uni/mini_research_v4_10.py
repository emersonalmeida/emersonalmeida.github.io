...
# -*- coding: utf-8 -*-
"""
Mini Research v4.10 - Coletor de Dados Multi-Fonte com Análises e Dashboard
Versão refinada com padronização completa e UX/UI/CLI otimizada

Melhorias v4.10:
- Padronização completa: menus, mensagens, exibição, salvamento unificados
- Remoção total de símbolos excessivos de layout
- UX/UI/CLI otimizada: experiência fluida e intuitiva
- Código limpo: redundâncias e repetições removidas
- Mensagens consistentes: padrão único em todo o script
- Exibição padronizada: formato uniforme para todos os dados
- Salvamento unificado: estrutura consistente para todos os CSVs
- Relatórios padronizados: formato único para estatísticas e insights
- Melhorias de usabilidade: mensagens mais concisas e claras

Melhorias v4.9 (mantidas):
- Padronização completa: menus, mensagens, exibição, salvamento unificados
- Remoção de símbolos excessivos de layout (####, ====, ----)
- UX/UI/CLI melhorada: experiência fluida de ponta a ponta
- Código limpo: remoção de redundâncias e repetições
- Mensagens consistentes: padrão único em todo o script
- Exibição padronizada: formato uniforme para todos os dados
- Salvamento unificado: estrutura consistente para todos os CSVs
- Relatórios padronizados: formato único para estatísticas e insights

Melhorias v4.8 (mantidas):
- Idioma automático por região: mapeamento inteligente (br→pt-BR, us→en-US, etc)
- Coleta múltipla de ordenações: YouTube vídeos e comentários coletados para CADA opção selecionada
- Coleta múltipla de reviews: App Store reviews coletadas para CADA opção de ordenação selecionada
- Respeitar ordem de seleção: dados exibidos na ordem que o usuário selecionou as opções
- Máximo de resultados: quando solicitado número muito grande, traz o máximo possível
- SERP individual: coleta resultados separados por buscador (padrão 20 por buscador)
- Reviews individuais: coleta reviews separadas por app (padrão 100 por app)
- Seção OUTRAS FONTES: estrutura preparada para futuras fontes seguindo o mesmo padrão
- Menus melhorados: descrições mais claras e informativas conforme padrão do usuário

Melhorias v4.7 (mantidas):
...
def parse_todos_input(input_str: str, options_dict: Dict, default: Optional[int] = None) -> List[int]:
    """Função unificada para parse de input com suporte 'todos'"""
...
def safe_int_input(prompt: str, default: int, min_val: Optional[int] = None, max_val: Optional[int] = None) -> int:
    """Parse seguro de input inteiro com validação"""
    while True:
        try:
...
def safe_float_input(prompt: str, default: float, min_val: Optional[float] = None, max_val: Optional[float] = None) -> float:
    """Parse seguro de input float com validação"""
    while True:
        try:
            value_str = input(prompt).strip()
            if not value_str:
...
def sanitize_term(term: str) -> str:
    """Sanitiza termo de busca com validação"""
...
def safe_save_csv(df: pd.DataFrame, filepath: str, backup: bool = True) -> bool:
    """Salva CSV com validação e backup"""
    try:
        # Validar DataFrame
...
def validate_data_before_consolidate(data: Any, source_name: str) -> bool:
    """Valida dados antes de consolidar"""
    if data is None:
...
def validate_api_keys() -> Dict[str, bool]:
    """Valida disponibilidade de API keys"""
    keys_status = {
...
def print_progress(message, icon="⏳"):
    """Mensagem de progresso padronizada"""
...
def print_success(message, count=None):
    ...
    if count is not None:
        print(f"{green('✓')} {message} {gray(f'({count} itens)')}")
    else:
...
def print_data_item_tqdm(index, item, prefix="", pbar=None):
    """Exibe item de dados com tqdm padronizado"""
    if pbar:
        index_str = green(f"{index:2d}.")
        if prefix and "\033[" not in prefix:
            prefix = gray(prefix) if prefix.strip() else prefix
...
# CONSTANTES E MAPEAMENTOS CENTRALIZADOS
...
# Mapeamento automático de idioma por região
REGION_LANG_MAP = {
    "br": "pt-BR",
    "us": "en-US",
    "fr": "fr-FR",
...
def get_lang_for_region(region: str) -> str:
    """Retorna idioma apropriado para a região"""
    return REGION_LANG_MAP.get(region.lower(), "pt-BR")
...
# Mapeamento de nomes de fontes para exibição
FONTES_NOMES_EXIBICAO = {
    1: "Suggest",
    2: "Trends",
    3: "SERP",
    4: "YouTube",
...
def make_session():
    """Cria sessão HTTP com retry compatível com diferentes versões do urllib3"""
...
def get_suggestions(query, region="br", client="chrome", source="", lang="", limit=10):
    """Consulta a API do Google Suggest com cache e tratamento de erros"""
    ...
    for attempt in range(max_retries):
        try:
            ...
            # v4.8: Se limit muito grande, retornar máximo possível
            max_possible = 1000  # Limite prático do Google Suggest
...
def run_suggest(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> List[Dict]:
    ...
    print(f"\n{cyan(bold('GOOGLE SUGGEST'))} - {termo}\n")
    ...
    if HAS_PLAY_SCRAPER:  # tqdm disponível
        pbar = tqdm(total=total_iteracoes, desc="Sugestões", unit="req")
    else:
        pbar = None
        print_progress("Coletando sugestões")
...
def run_trends(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> List[Dict]:
    """Executa Google Trends com múltiplos períodos"""
    if not HAS_PYTRENDS:
        logger.warning("pytrends não instalado. Pulando Google Trends.")
        print(f"{yellow('[!]')} pytrends não instalado. Pulando Google Trends.")
        return []
    
    print(f"\n{cyan(bold('GOOGLE TRENDS'))} - {termo}\n")
    
    region = REGION_MAP_TRENDS.get(config.get("region", "br"), "BR")
    lang = config.get("lang", "pt-BR")  # v4.8: usar idioma mapeado
    ...
    timeframes = config.get("timeframes", [4])
    opcoes = config.get("opcoes", [1, 2, 3, 4])
    opcoes_order = config.get("opcoes_order", opcoes)
    topn = config.get("topn", 20)
    delay = config.get("delay", 1.0)
    
    # Buscar máximo possível se solicitado número muito grande
    max_possible = 10000
    topn_efetivo = min(topn, max_possible)
    
    OUTPUT_DIR = ensure_dir(os.path.join(output_dir, "trends"))
    resultados = []
    tipos_nome_map = {"": "web", "images": "images", "news": "news", "youtube": "youtube"}
    
    # v4.8: Processar para cada tipo de fonte e cada período selecionado
    for gtype in gtypes:
        tipo_nome = tipos_nome_map.get(gtype, "web")
        
        # Processar múltiplos períodos
        for periodo_idx, periodo_id in enumerate(timeframes, 1):
            timeframe = periodos_map.get(periodo_id, "today 12-m")
            periodo_nome_map = {1: "7 dias", 2: "1 mês", 3: "3 meses", 4: "12 meses", 5: "5 anos", 6: "Todo período"}
            periodo_nome = periodo_nome_map.get(periodo_id, "12 meses")
            
            print_progress(f"{tipo_nome} - {periodo_nome} ({periodo_idx}/{len(timeframes)})")
            ...
            try:
                ...
                logger.error(f"Erro ao construir payload do Trends ({tipo_nome}, {periodo_nome}): {e}")
                print(red(f"  [ERRO] {e}"))
                continue
            
            # Processar opções na ordem selecionada
            for opcao_idx, opcao in enumerate(opcoes_order, 1):
                if opcao == 1:
                    try:
                        related = pytrends.related_queries()
                        r = related.get(termo, {})
                        if r and "top" in r and r["top"] is not None:
                            df = r["top"].head(topn_efetivo).copy()
                            print_progress(f"Top relacionados: {len(df)} itens")
                            for i, row in enumerate(df.itertuples(), 1):
                                ...
                                periodo_info = gray(f"[{periodo_nome}]")
                                print_data_item(i, query_text, f"{value_text} {periodo_info} ")
                            file = os.path.join(OUTPUT_DIR, f"top_{tipo_nome}_{periodo_nome.replace(' ', '_')}_{termo}_{now_tag()}.csv")
                            if safe_save_csv(df, file):
                                print_success(f"{file}", len(df))
                            else:
                                logger.warning(f"Falha ao salvar: {file}")
                    except Exception as e:
                        logger.warning(f"Erro ao processar top relacionados ({tipo_nome}, {periodo_nome}): {e}")
                        pass
                    ...
                    try:
                        ...
                        if r and "rising" in r and r["rising"] is not None:
                            df = r["rising"].head(topn_efetivo).copy()
                            print_progress(f"Rising relacionados: {len(df)} itens")
                            for i, row in enumerate(df.itertuples(), 1):
                                item = {
                                    "tipo": "rising", 
                                    "fonte": tipo_nome, 
                                ...
                                periodo_info = gray(f"[{periodo_nome}]")
                                print_data_item(i, query_text, f"{value_text} {periodo_info} ")
                            file = os.path.join(OUTPUT_DIR, f"rising_{tipo_nome}_{periodo_nome.replace(' ', '_')}_{termo}_{now_tag()}.csv")
                            if safe_save_csv(df, file):
                                print_success(f"{file}", len(df))
                    except Exception as e:
                        logger.warning(f"Erro ao processar rising relacionados ({tipo_nome}, {periodo_nome}): {e}")
                        pass
                    ...
                    try:
                        ...
                        if not regioes.empty:
                            serie = regioes[termo].sort_values(ascending=False).head(topn_efetivo)
                            print_progress(f"Interesse por regiões: {len(serie)} itens")
                            for i, (reg, val) in enumerate(serie.items(), 1):
                                ...
                                val_text = gray(f"({val})")
                                periodo_info = gray(f"[{periodo_nome}]")
                                print_data_item(i, reg_text, f"{val_text} {periodo_info} ")
                            df = pd.DataFrame({"regiao": serie.index, "valor": serie.values})
                            file = os.path.join(OUTPUT_DIR, f"regioes_{tipo_nome}_{periodo_nome.replace(' ', '_')}_{termo}_{now_tag()}.csv")
                            if safe_save_csv(df, file):
                                print_success(f"{file}", len(serie))
                    except Exception as e:
                        logger.warning(f"Erro ao processar interesse por regiões ({tipo_nome}, {periodo_nome}): {e}")
                        pass
                    ...
                    try:
                        ...
                        if not df_time.empty:
                            print_progress(f"Interesse ao longo do tempo: {len(df_time)} pontos")
                            for i, (idx, val) in enumerate(df_time[termo].items(), 1):
                                ...
                                val_text = gray(f"→ {val}")
                                periodo_info = gray(f"[{periodo_nome}]")
                                print_data_item(i, date_text, f" {val_text} {periodo_info}")
                            df = pd.DataFrame({"data": df_time.index, "valor": df_time[termo].values})
                            file = os.path.join(OUTPUT_DIR, f"tempo_{tipo_nome}_{periodo_nome.replace(' ', '_')}_{termo}_{now_tag()}.csv")
                            if safe_save_csv(df, file):
                                print_success(f"{file}", len(df_time))
                    except Exception as e:
                        logger.warning(f"Erro ao processar interesse ao longo do tempo ({tipo_nome}, {periodo_nome}): {e}")
                        pass
...
def run_serp(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> List[Dict]:
    """Executa SERP com coleta individual por buscador"""
    print(f"\n{cyan(bold('SERP'))} - {termo}\n")
    
    region = config.get("region", "br")
    lang = config.get("lang", "pt-BR")
    limite = config.get("limite", 20)
    limite_por_buscador = config.get("limite_por_buscador", limite)
    buscadores = config.get("buscadores", [1])
    buscadores_order = config.get("order", buscadores)
    delay = config.get("delay", 1.0)
    ...
    # Coletar resultados individuais por buscador
    for buscador_idx, bus_id in enumerate(buscadores_order, 1):
        ...
        if not disponivel:
            print(f"{yellow('[!]')} {nome} não disponível (faltam API keys)")
            continue
        
        print_progress(f"{nome} ({buscador_idx}/{len(buscadores_order)})")
        
                            # Buscar máximo possível se solicitado número muito grande
        max_possible = 1000
        limite_efetivo = min(limite_por_buscador, max_possible)
        
        if bus_id == 2:
            res = func(termo, region, lang, limite_efetivo)
        else:
            res = func(termo, region, limite_efetivo)
        
        # Garantir que temos exatamente o número solicitado (ou máximo possível)
        res = res[:limite_efetivo]
        
        time.sleep(delay)
        
        if res:
            print_progress(f"{nome}: {len(res)} resultados")
            for r in res:
                resultados.append(r)
            ...
            # Salvar resultados individuais por buscador
            df = pd.DataFrame(res)
...
def buscar_videos_api(query, region, lang, order=1, max_results=10):
    """Busca vídeos via API oficial com todos os metadados"""
    if not youtube:
        return []
    try:
        ...
        if video_ids:
            try:
                # Buscar todos os parts disponíveis
        ...
        for item in response.get("items", []):
            ...
            videos.append({
                ...
                "defaultLanguage": detalhes.get("defaultLanguage", ""),
                "defaultAudioLanguage": detalhes.get("defaultAudioLanguage", ""),
                "liveBroadcastContent": detalhes.get("liveBroadcastContent", ""),
                "topicIds": detalhes.get("topicIds", []),
                "topicCategories": detalhes.get("topicCategories", []),
                "recordingDate": detalhes.get("recordingDate", ""),
                "locationDescription": detalhes.get("locationDescription", ""),
                "actualStartTime": detalhes.get("actualStartTime", ""),
                "actualEndTime": detalhes.get("actualEndTime", ""),
                "scheduledStartTime": detalhes.get("scheduledStartTime", ""),
                "scheduledEndTime": detalhes.get("scheduledEndTime", ""),
                "concurrentViewers": detalhes.get("concurrentViewers", ""),
                "activeLiveChatId": detalhes.get("activeLiveChatId", ""),
...
def buscar_videos_scraping(query, max_results=10):
    """Busca vídeos via scraping com todos os metadados"""
...
def buscar_comentarios_api(video_id, max_results=20):
    """Busca comentários via API oficial com todos os metadados"""
    if not youtube:
        return []
    try:
...
def buscar_comentarios_scraping(video_id, max_results=20):
    """Busca comentários via scraping com todos os metadados"""
    if not HAS_YOUTUBE_SEARCH:
        return []
    try:
...
def ordenar_comentarios(comentarios, criterio=1):
    """Ordena comentários por critério"""
    if not comentarios:
        return comentarios
...
def run_youtube(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> Dict:
    """Executa YouTube com coleta múltipla de ordenações"""
    print(f"\n{cyan(bold('YOUTUBE'))} - {termo}\n")
    
    region = config.get("region", "br")
    lang = config.get("lang", "pt-BR")
    orders = config.get("orders", [1])
    orders_order = config.get("orders_order", orders)
    limite_videos = config.get("limite_videos", 50)
    limite_videos_por_ordenacao = config.get("limite_videos_por_ordenacao", limite_videos)
    coletar_comentarios = config.get("coletar_comentarios", False)
    limite_comentarios = config.get("limite_comentarios", 50)
    limite_comentarios_por_ordenacao = config.get("limite_comentarios_por_ordenacao", limite_comentarios)
    videos_selecionados = config.get("videos_selecionados", [1])
    order_comentarios_list = config.get("order_comentarios_list", [1])
    comment_orders_order = config.get("comment_orders_order", order_comentarios_list)
    delay = config.get("delay", 1.0)
    
    YOUTUBE_DIR = ensure_dir(os.path.join(output_dir, "youtube"))
    
    # Coletar vídeos para cada ordenação selecionada
    videos_todos = []
    order_map = {1: "relevance", 2: "date", 3: "viewCount"}
    order_names = {1: "Relevância", 2: "Data de publicação", 3: "Número de visualizações"}
    
    for order_idx, order in enumerate(orders_order, 1):
        order_str = order_map.get(order, "relevance")
        order_name = order_names.get(order, "Relevância")
        print_progress(f"Vídeos por {order_name} ({order_idx}/{len(orders_order)})")
        
                            # Buscar máximo possível se solicitado número muito grande
        max_possible = 1000  # Limite prático da API
        limite_efetivo = min(limite_videos_por_ordenacao, max_possible)
        
        videos = buscar_videos(termo, region, lang, order, limite_efetivo)
        time.sleep(delay)
        
        if videos:
            # Garantir que temos exatamente o número solicitado (ou máximo possível)
            videos = videos[:limite_efetivo]
            # Adicionar metadado de ordenação
            for v in videos:
                v["ordenacao"] = order
                v["ordenacao_nome"] = order_name
            videos_todos.extend(videos)
            print_progress(f"{order_name}: {len(videos)} vídeos")
    
    # Remover vídeos duplicados (mesmo videoId) mantendo primeira ocorrência
    videos_unicos = {}
    for v in videos_todos:
        video_id = v.get("videoId", "")
        if video_id and video_id not in videos_unicos:
            videos_unicos[video_id] = v
    
    videos = list(videos_unicos.values())
    
    if not videos:
        print(f"{yellow('[!]')} Nenhum vídeo encontrado")
        return {}
    
    print_progress(f"Total de vídeos únicos: {len(videos)}")
    
    # Exibir vídeos com contadores e metadados expandidos (respeitando ordem de ordenação)
    for i, v in enumerate(videos, 1):
        item = {"video": v, "tipo": "video"}
        resultados_tempo_real["youtube"].append(item)
        ordenacao_info = gray(f"[{v.get('ordenacao_nome', 'N/A')}]")
        ...
        print_data_item(i, titulo_text, f"[{green(f'{i}/{len(videos)}')}] {ordenacao_info} ")
    ...
    if safe_save_csv(df_videos, file):
        print_success(f"Vídeos: {file}", len(videos))
    ...
    if coletar_comentarios:
        ...
        # v4.8: Coletar comentários para CADA vídeo com CADA ordenação selecionada
        for i in indices_list:
            if i <= len(videos):
                video_idx += 1
                vid = videos[i - 1]
                
                # v4.8: Para cada ordenação de comentários selecionada
                for comment_order_idx, comment_order in enumerate(comment_orders_order, 1):
                    comment_order_name_map = {1: "Mais novos", 2: "Mais antigos", 3: "Mais curtidos", 4: "Mais longos"}
                    comment_order_name = comment_order_name_map.get(comment_order, "Mais novos")
                    
                    print_progress(f"Vídeo {video_idx}/{total_videos_comentarios} - {comment_order_name} ({comment_order_idx}/{len(comment_orders_order)}): {vid['titulo']}")
                    
                    # Mapear ordenação para API
                    order_map_comments = {1: "time", 2: "time", 3: "relevance", 4: "relevance"}
                    order_str = order_map_comments.get(comment_order, "relevance")
                    
                    # Buscar máximo possível se solicitado número muito grande
                    max_possible_comments = 1000  # Limite prático
                    limite_efetivo_comments = min(limite_comentarios_por_ordenacao, max_possible_comments)
                    
                    comentarios = buscar_comentarios(vid["videoId"], limite_efetivo_comments, order_str)
                    # Aplicar ordenação adicional (especialmente para longest)
                    comentarios = ordenar_comentarios(comentarios, comment_order)
                    # Garantir que temos exatamente o número solicitado (ou máximo possível)
                    comentarios = comentarios[:limite_efetivo_comments]
                    time.sleep(delay)
                    ...
                    if comentarios:
                        for j, c in enumerate(comentarios, 1):
                            ...
                            c["ordenacao_comentario"] = comment_order  # v4.8: metadado de ordenação
                            c["ordenacao_comentario_nome"] = comment_order_name  # v4.8
                            ...
                            comentario_text = str(c['comentario'])
                            ...
                            ordenacao_comment_info = gray(f"[{comment_order_name}]")
                            print_data_item(j, comentario_text, f"{autor_text}: {ordenacao_comment_info} ")
        ...
        if comentarios_todos:
            ...
            if safe_save_csv(df_comentarios, file):
                print_success(f"Comentários: {file}", len(comentarios_todos))
...
def apple_df(results):
    """Converte resultados da App Store em DataFrame com todos os metadados"""
    rows = []
    for r in results:
        rows.append({
            ...
            "screenshotUrls": str(r.get("screenshotUrls", [])),
            "ipadScreenshotUrls": str(r.get("ipadScreenshotUrls", [])),
            "appletvScreenshotUrls": str(r.get("appletvScreenshotUrls", [])),
            ...
            "languageCodesISO2A": str(r.get("languageCodesISO2A", [])),
...
def fetch_google(term, lang="pt", country="br", n=20):
    """Busca apps do Google Play com todos os metadados"""
    if not HAS_PLAY_SCRAPER:
        return pd.DataFrame()
    try:
    ...
    for r in res[:n]:
        rows.append({
            ...
            "url": r.get("url", ""),
            "icon": r.get("icon", ""),
            "headerImage": r.get("headerImage", ""),
            "screenshots": str(r.get("screenshots", [])),
            "video": r.get("video", ""),
            "videoImage": r.get("videoImage", ""),
            "price": r.get("price", ""),
            "free": r.get("free", True),
            "contentRating": r.get("contentRating", ""),
            "contentRatingDescription": r.get("contentRatingDescription", ""),
            "genre": r.get("genre", ""),
            "genreId": r.get("genreId", ""),
            "familyGenre": r.get("familyGenre", ""),
            "familyGenreId": r.get("familyGenreId", ""),
            "summary": r.get("summary", ""),
            "description": r.get("description", ""),
            "descriptionHTML": r.get("descriptionHTML", ""),
            ...
            "developerAddress": r.get("developerAddress", ""),
            "privacyPolicy": r.get("privacyPolicy", ""),
            "histogram": str(r.get("histogram", {})),
            "offersIAP": r.get("offersIAP", False),
            "adSupported": r.get("adSupported", False),
            "recentChanges": r.get("recentChanges", ""),
            "permissions": str(r.get("permissions", [])),
            "whatsNew": r.get("whatsNew", "")
        })
    return pd.DataFrame(rows)

def fetch_reviews_google(app_id, lang="pt", country="br", max_reviews=200, sort_order=Sort.NEWEST):
    """Busca reviews do Google Play com todos os metadados"""
    if not HAS_PLAY_SCRAPER:
        return pd.DataFrame()
    out, token = [], None
    try:
...
def run_stores(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> Dict:
    """Executa App Stores com coleta múltipla de ordenações"""
    if not HAS_PLAY_SCRAPER:
        logger.warning("google-play-scraper não instalado. Pulando App Stores.")
        print(f"{yellow('[!]')} google-play-scraper não instalado. Pulando App Stores.")
        return {}
    
    print(f"\n{cyan(bold('APP STORES'))} - {termo}\n")
    
    country = config.get("country", "br")
    lang = config.get("lang", "pt-BR")
    n_apps = config.get("n_apps", 50)
    max_reviews = config.get("max_reviews", 50)
    max_reviews_por_ordenacao = config.get("max_reviews_por_ordenacao", max_reviews)
    coletar_reviews = config.get("coletar_reviews", False)
    apps_selecionados = config.get("apps_selecionados", [1, 2, 3])
    order_reviews_list = config.get("order_reviews_list", [1])
    review_orders_order = config.get("review_orders_order", order_reviews_list)
    lojas = config.get("lojas", [1, 2])
    lojas_order = config.get("lojas_order", lojas)
    delay = config.get("delay", 1.0)
    
    STORES_DIR = ensure_dir(os.path.join(output_dir, "stores"))
    resultados = {}
    
    # Processar lojas na ordem selecionada
    for loja_idx, loja_id in enumerate(lojas_order, 1):
        if loja_id == 1:
            print_progress("Google Play")
            # Buscar máximo possível se solicitado número muito grande
            max_possible_apps = 1000
            n_apps_efetivo = min(n_apps, max_possible_apps)
            
            df_google = fetch_google(termo, lang, country, n_apps_efetivo)
            time.sleep(delay)
            
            if not df_google.empty:
                # Garantir que temos exatamente o número solicitado (ou máximo possível)
                df_google = df_google.head(n_apps_efetivo)
                print_progress(f"Google Play: {len(df_google)} apps")
                ...
                if safe_save_csv(df_google, file):
                    print_success(f"Apps Google Play: {file}", len(df_google))
                resultados["google_play"] = df_google
                
                if coletar_reviews:
                    print_progress("Reviews do Google Play")
                    ...
                    # v4.8: Para cada app, coletar reviews para CADA ordenação selecionada
                    for app_id in apps_ids:
                        app_idx += 1
                        app_title = df_google.loc[df_google["id"] == app_id, "title"].iloc[0]
                        
                        # v4.8: Coletar reviews para cada ordenação
                        for review_order_idx, review_order in enumerate(review_orders_order, 1):
                            review_order_name_map = {1: "Mais novos", 2: "Mais antigos", 3: "Melhores avaliadas", 4: "Piores avaliadas", 5: "Mais votadas"}
                            review_order_name = review_order_name_map.get(review_order, "Mais novos")
                            
                            print_progress(f"  App {app_idx}/{total_apps_reviews} - {review_order_name} ({review_order_idx}/{len(review_orders_order)}): {app_title}...")
                            
                            # Mapear ordenação do Google Play
                            sort_map = {1: Sort.NEWEST, 2: Sort.MOST_RELEVANT, 3: Sort.RATING}
                            sort_order = sort_map.get(review_order, Sort.NEWEST)
                            
                            # Buscar máximo possível se solicitado número muito grande
                            max_possible_reviews = 10000
                            max_reviews_efetivo = min(max_reviews_por_ordenacao, max_possible_reviews)
                            
                            df_reviews = fetch_reviews_google(app_id, lang, country, max_reviews_efetivo, sort_order)
                            # Aplicar ordenação adicional se necessário
                            df_reviews = ordenar_reviews(df_reviews, review_order)
                            # Garantir que temos exatamente o número solicitado (ou máximo possível)
                            df_reviews = df_reviews.head(max_reviews_efetivo)
                            time.sleep(delay)
                            
                            if not df_reviews.empty:
                                # Adicionar metadado de ordenação
                                df_reviews["ordenacao"] = review_order
                                df_reviews["ordenacao_nome"] = review_order_name
                                
                                # Exibir reviews em tempo real
                                for j, row in enumerate(df_reviews.itertuples(), 1):
                                    item = {
                                        "review": {
                                            ...
                                            "ordenacao": review_order,
                                            "ordenacao_nome": review_order_name
                                        }, 
                                        "tipo": "review", 
                                        "loja": "google_play"
                                    }
                                    resultados_tempo_real["stores"].append(item)
                                    # Verde para rating, branco para conteúdo
                                    ...
                                    ordenacao_info = gray(f"[{review_order_name}]")
                                    print_data_item(j, content_text, f"{rating_text} | {title_str}{ordenacao_info} ")
                                    print(f"      {votes_str}")
                                ...
                                file = os.path.join(STORES_DIR, f"reviews_google_{app_id}_{review_order_name.replace(' ', '_').lower()}.csv")
                                if safe_save_csv(df_reviews, file):
                                    print_success(f"  Reviews ({review_order_name}): {file}", len(df_reviews))
            else:
                resultados["google_play"] = pd.DataFrame()
        
        elif loja_id == 2:
            print_progress("App Store")
            # Buscar máximo possível se solicitado número muito grande
            max_possible_apps = 1000
            n_apps_efetivo = min(n_apps, max_possible_apps)
            
            df_apple = apple_df(fetch_apple(termo, country, n_apps_efetivo))
            time.sleep(delay)
            
            if not df_apple.empty:
                # Garantir que temos exatamente o número solicitado (ou máximo possível)
                df_apple = df_apple.head(n_apps_efetivo)
                print_progress(f"App Store: {len(df_apple)} apps")
                ...
                if safe_save_csv(df_apple, file):
                    print_success(f"Apps App Store: {file}", len(df_apple))
                resultados["app_store"] = df_apple
                
                if coletar_reviews:
                    print_progress("Reviews da App Store")
                    if apps_selecionados == "todos" or apps_selecionados == "t":
                    ...
                    for app_id in apps_ids:
                        ...
                        for review_order_idx, review_order in enumerate(review_orders_order, 1):
                            ...
                            df_reviews = fetch_reviews_apple(app_id, country, max_reviews_efetivo)
                            # Aplicar ordenação
                            df_reviews = ordenar_reviews(df_reviews, review_order)
                            # Garantir que temos exatamente o número solicitado (ou máximo possível)
                            df_reviews = df_reviews.head(max_reviews_efetivo)
                            ...
                            if not df_reviews.empty:
                                ...
                                file = os.path.join(STORES_DIR, f"reviews_apple_{app_id}_{review_order_name.replace(' ', '_').lower()}.csv")
                                if safe_save_csv(df_reviews, file):
                                    print_success(f"  Reviews ({review_order_name}): {file}", len(df_reviews))
            else:
...
def gerar_estatisticas(df_consolidado, termo, output_dir, fontes_ordenadas=None):
    """Gera estatísticas descritivas avançadas dos dados coletados"""
    if df_consolidado.empty:
    ...
    try:
        ...
        print_success(f"Estatísticas TXT: {stats_file_txt}")
    ...
    try:
        ...
        print_success(f"Estatísticas JSON: {stats_file_json}")
...
def gerar_graficos(resultados, termo, output_dir):
    ...
    if not HAS_MATPLOTLIB:
        print(f"{yellow('[!]')} matplotlib não disponível. Pulando geração de gráficos.")
        return
...
def gerar_insights(resultados, termo, output_dir):
    ...
    print_success(f"Insights: {insights_file}")
    
    return insights

def exibir_dashboard(resultados, termo, output_dir, stats, insights, fontes_ordenadas=None):
    """Exibe dashboard completo e detalhado com cores padronizadas - v4.8"""
    print(f"\n{cyan(bold(f'DASHBOARD - {termo.upper()}'))}\n")
    ...
    print(f"\n{cyan(bold('ESTATÍSTICAS POR FONTE'))}")
    ...
    print(f"\n{green(bold('Dashboard gerado com sucesso'))}\n")

# CONFIGURAÇÃO

def coletar_config_fonte(fonte_id: int, config: Dict) -> Dict:
    """Coleta configuração específica de uma fonte"""
    fonte_config = {}
    
    if fonte_id == 1:  # Google Suggest
        print(f"\n{cyan(bold('04. Suggest'))}")
        ...
        limit = safe_int_input("  Resultados [15]: ", 15, min_val=1, max_val=10000)
        fonte_config = {
            "clients": clients_selected, 
            "sources": sources_selected, 
            "opcoes": opcoes_selected, 
            "limit": limit,
            "order": opcoes_selected  # v4.8: manter ordem de seleção
        }
    
    elif fonte_id == 2:  # Google Trends
        print(f"\n{cyan(bold('05. Trends'))}")
        print_menu("  Fontes", "", TRENDS_TIPOS, default=1)
        tipos_selected = parse_numeric_input(input("  > [1,2]: ").strip() or "1,2", TRENDS_TIPOS, default=1)
        print_menu("  Períodos", "", TRENDS_PERIODOS, default=4)
        periodos_selected = parse_numeric_input(input("  > [2,4,6]: ").strip() or "2,4,6", TRENDS_PERIODOS, default=4)
        print_menu("  Dados", "", TRENDS_OPCOES, default=1)
        opcoes_selected = parse_todos_input(input("  > [t]: ").strip() or "t", TRENDS_OPCOES, default=1)
        topn = safe_int_input("  Resultados [20]: ", 20, min_val=1, max_val=10000)  # v4.8: aumentar limite máximo
        # v4.8: Manter múltiplos períodos e ordem de seleção
        fonte_config = {
            "gtypes": tipos_selected, 
            "timeframes": periodos_selected,  # v4.8: múltiplos períodos
            "timeframe": periodos_selected[0] if periodos_selected else 4,  # Para compatibilidade
            "opcoes": opcoes_selected, 
            "opcoes_order": opcoes_selected,  # v4.8: manter ordem de seleção
            "topn": topn
        }
        ...
        limite = safe_int_input("  Resultados [20]: ", 20, min_val=1, max_val=1000)
        fonte_config = {
            "buscadores": buscadores_selected, 
            "limite": limite,
            "limite_por_buscador": limite,  # v4.8: limite por buscador
            "order": buscadores_selected  # v4.8: manter ordem de seleção
        }
    
    elif fonte_id == 4:  # YouTube
        print(f"\n{cyan(bold('07. YouTube'))}")
        limite_videos = safe_int_input("  Vídeos [10]: ", 10, min_val=1, max_val=1000)  # v4.8: aumentar limite
        ...
        # v4.8: Coletar vídeos para CADA opção de ordenação selecionada
        coletar_comentarios = input("  Coletar comentários? (s/n) [s]: ").strip().lower() or "s"
        limite_comentarios = 0
        order_comentarios_list = []
        if coletar_comentarios == "s":
            limite_comentarios = safe_int_input("  Comentários por vídeo [20]: ", 20, min_val=1, max_val=1000)
            print_menu("  Ordenação comentários", "", YOUTUBE_COMMENT_ORDER, default=1)
            comment_order_selected = parse_todos_input(input("  > [t]: ").strip() or "t", YOUTUBE_COMMENT_ORDER, default=1)
            # v4.8: Coletar comentários para CADA opção de ordenação selecionada
            order_comentarios_list = comment_order_selected if comment_order_selected else [1]
        fonte_config = {
            "orders": order_selected,  # v4.8: múltiplas ordenações
            "order": order_selected[0] if order_selected else 1,  # Para compatibilidade
            "limite_videos": limite_videos,
            "limite_videos_por_ordenacao": limite_videos,  # v4.8: limite por ordenação
            "coletar_comentarios": coletar_comentarios == "s",
            "limite_comentarios": limite_comentarios,
            "limite_comentarios_por_ordenacao": limite_comentarios,
            "order_comentarios": order_comentarios_list[0] if order_comentarios_list else 1,  # Para compatibilidade
            "order_comentarios_list": order_comentarios_list,  # v4.8: múltiplas ordenações
            "videos_selecionados": "todos",
            "orders_order": order_selected,  # v4.8: manter ordem de seleção
            "comment_orders_order": comment_order_selected if coletar_comentarios == "s" else []  # v4.8: manter ordem
        }
    
    elif fonte_id == 5:  # App Stores
        print(f"\n{cyan(bold('08. App Store'))}")
        ...
        n_apps = safe_int_input("  Aplicativos [20]: ", 20, min_val=1, max_val=1000)  # v4.8: aumentar limite
        coletar_reviews = input("  Coletar reviews? (s/n) [s]: ").strip().lower() or "s"
        max_reviews = 0
        order_reviews_list = []
        if coletar_reviews == "s":
            max_reviews = safe_int_input("  Reviews por app [20]: ", 20, min_val=1, max_val=10000)
            print_menu("  Ordenação reviews", "", APP_STORE_REVIEW_ORDER, default=1)
            review_order_selected = parse_todos_input(input("  > [t]: ").strip() or "t", APP_STORE_REVIEW_ORDER, default=1)
            # v4.8: Coletar reviews para CADA opção de ordenação selecionada
            order_reviews_list = review_order_selected if review_order_selected else [1]
        fonte_config = {
            "lojas": lojas_selected,
            "lojas_order": lojas_selected,  # v4.8: manter ordem de seleção
            "n_apps": n_apps,
            "coletar_reviews": coletar_reviews == "s",
            "max_reviews": max_reviews,
            "max_reviews_por_ordenacao": max_reviews,
            "order_reviews": order_reviews_list[0] if order_reviews_list else 1,  # Para compatibilidade
            "order_reviews_list": order_reviews_list,  # v4.8: múltiplas ordenações
            "apps_selecionados": "todos",
            "review_orders_order": review_order_selected if coletar_reviews == "s" else []
        }
    
    return fonte_config

def coletar_configuracao() -> Optional[Dict]:
    """Coleta configuração otimizada v4.8"""
    print(f"\n{cyan(bold('Mini Research v4.10 - Configuração'))}\n")
    ...
    if not termos_input:
        print(f"{red('Erro:')} Termo não pode estar vazio!")
        return None
    ...
    if not termos:
        print(f"{red('Erro:')} Nenhum termo válido encontrado após sanitização!")
        return None
    ...
    config["langs"] = [get_lang_for_region(REGION_MAP[r]) for r in regions_selected if r in REGION_MAP]
    # Usar primeiro idioma como padrão, mas manter lista para múltiplas regiões
    config["lang"] = config["langs"][0] if config["langs"] else "pt-BR"
    
    # 03. PLATAFORMAS (com descrições melhoradas - v4.8)
    ...
    # Aplicar parâmetros globais com idioma automático
    for fonte_config in ["suggest", "trends", "serp", "youtube", "stores"]:
        if fonte_config in config:
            ...
            # Usar idioma mapeado automaticamente
            config[fonte_config]["lang"] = config.get("lang", "pt-BR")
            config[fonte_config]["langs"] = config.get("langs", ["pt-BR"])
    ...
    # v4.8: Preparar estrutura para OUTRAS FONTES (futuras implementações)
    # Esta seção pode ser expandida no futuro seguindo o mesmo padrão
    ...
    confirm = input(f"\n{cyan(bold('Iniciar coleta?'))} (s/n) [s]: ").strip().lower() or "s"
    if confirm != "s":
        print(f"{yellow('Coleta cancelada.')}")
        return None
...
def main():
    """Função principal otimizada v4.8"""
    print(f"\n{cyan(bold('Mini Research v4.10 - Coletor de Dados Multi-Fonte'))}\n")
    ...
    if missing:
        ...
        print(f"{yellow('Aviso:')} Algumas API keys estão ausentes: {', '.join(missing)}")
    ...
    print(f"\n{green(bold('COLETA FINALIZADA'))}\n")
    print(f"{gray('Dados salvos em:')} {cyan(output_dir)}\n")
