...
# -*- coding: utf-8 -*-
"""
Mini Research v5.1 - Coletor de Dados Multi-Fonte com Análises e Dashboard
...
# Mapeamento automático de idioma por região - v4.8
REGION_LANG_MAP = {
    "br": "pt-BR",
    "us": "en-US",
    "fr": "fr-FR",
...
def get_lang_for_region(region: str) -> str:
    """Retorna idioma apropriado para a região - v4.8"""
...
# Mapeamento de nomes de fontes para exibição - v4.8
...
def run_trends(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> List[Dict]:
    """Executa Google Trends com múltiplos períodos e recursos expandidos - v4.8"""
    if not HAS_PYTRENDS:
    ...
    timeframes = config.get("timeframes", [4])  # v4.8: múltiplos períodos
    opcoes = config.get("opcoes", [1, 2, 3, 4])
    opcoes_order = config.get("opcoes_order", opcoes)  # v4.8: manter ordem de seleção
    topn = config.get("topn", 20)
    delay = config.get("delay", 1.0)
    
    # v4.8: Buscar máximo possível se solicitado número muito grande
    max_possible = 10000
    ...
    for gtype in gtypes:
        ...
        # v4.8: Processar múltiplos períodos
        for periodo_idx, periodo_id in enumerate(timeframes, 1):
            ...
            print_progress(f"Processando {tipo_nome} - {periodo_nome} ({periodo_idx}/{len(timeframes)})...")
            ...
            # v4.8: Processar opções na ordem selecionada
                        ...
                        if r and "top" in r and r["top"] is not None:
                            ...
                            for i, row in enumerate(df.itertuples(), 1):
                                item = {
                                    "tipo": "top", 
                                    "fonte": tipo_nome, 
                                    "periodo": periodo_nome,  # v4.8: metadado de período
                                    "query": row.query, 
                                    "value": row.value
                                }
                                resultados.append(item)
...
def run_serp(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> List[Dict]:
    """Executa SERP com coleta individual por buscador - v4.8"""
    print(f"\n{cyan(bold('SERP'))} - {termo}\n")
    
    region = config.get("region", "br")
    lang = config.get("lang", "pt-BR")  # v4.8: usar idioma mapeado
    limite = config.get("limite", 20)
    limite_por_buscador = config.get("limite_por_buscador", limite)  # v4.8
    buscadores = config.get("buscadores", [1])
    buscadores_order = config.get("order", buscadores)  # v4.8: manter ordem de seleção
    delay = config.get("delay", 1.0)
    ...
    # v4.8: Coletar resultados individuais por buscador (respeitando ordem de seleção)
    for buscador_idx, bus_id in enumerate(buscadores_order, 1):
        ...
        if bus_id == 2:
            res = func(termo, region, lang, limite_efetivo)  # v4.8: usar idioma mapeado
        else:
            res = func(termo, region, limite_efetivo)
        
        # v4.8: Garantir que temos exatamente o número solicitado (ou máximo possível)
        ...
        if res:
            ...
            # Salvar resultados individuais por buscador - v4.8
            df = pd.DataFrame(res)
            file = os.path.join(SERP_DIR, f"{nome.lower()}_{termo}.csv")
            if safe_save_csv(df, file):
                print_success(f"{nome.capitalize()}: {file}", len(res))
...
def run_youtube(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> Dict:
    """Executa YouTube com coleta múltipla de ordenações - v4.8"""
    print(f"\n{cyan(bold('YOUTUBE'))} - {termo}\n")
    
    region = config.get("region", "br")
    lang = config.get("lang", "pt-BR")  # v4.8: usar idioma mapeado
    orders = config.get("orders", [1])  # v4.8: múltiplas ordenações
    orders_order = config.get("orders_order", orders)  # v4.8: manter ordem de seleção
    limite_videos = config.get("limite_videos", 50)
    limite_videos_por_ordenacao = config.get("limite_videos_por_ordenacao", limite_videos)  # v4.8
    coletar_comentarios = config.get("coletar_comentarios", False)
    limite_comentarios = config.get("limite_comentarios", 50)
    limite_comentarios_por_ordenacao = config.get("limite_comentarios_por_ordenacao", limite_comentarios)  # v4.8
    videos_selecionados = config.get("videos_selecionados", [1])
    order_comentarios_list = config.get("order_comentarios_list", [1])  # v4.8: múltiplas ordenações
    comment_orders_order = config.get("comment_orders_order", order_comentarios_list)  # v4.8: manter ordem
    delay = config.get("delay", 1.0)
    
    YOUTUBE_DIR = ensure_dir(os.path.join(output_dir, "youtube"))
    
    # v4.8: Coletar vídeos para CADA ordenação selecionada (respeitando ordem)
    videos_todos = []
    order_map = {1: "relevance", 2: "date", 3: "viewCount"}
    ...
    for order_idx, order in enumerate(orders_order, 1):
        ...
        print_progress(f"Buscando vídeos por {order_name} ({order_idx}/{len(orders_order)})...")
...
def run_stores(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> Dict:
    """Executa App Stores com coleta múltipla de ordenações - v4.8"""
    if not HAS_PLAY_SCRAPER:
    ...
    max_reviews_por_ordenacao = config.get("max_reviews_por_ordenacao", max_reviews)  # v4.8
    coletar_reviews = config.get("coletar_reviews", False)
    apps_selecionados = config.get("apps_selecionados", [1, 2, 3])
    order_reviews_list = config.get("order_reviews_list", [1])  # v4.8: múltiplas ordenações
    review_orders_order = config.get("review_orders_order", order_reviews_list)  # v4.8: manter ordem
    lojas = config.get("lojas", [1, 2])
    lojas_order = config.get("lojas_order", lojas)  # v4.8: manter ordem de seleção
    delay = config.get("delay", 1.0)
    
    STORES_DIR = ensure_dir(os.path.join(output_dir, "stores"))
    resultados = {}
    
    # v4.8: Processar lojas na ordem selecionada
...
def consolidar_dados(resultados: Dict, termo: str, output_dir: str, fontes_ordenadas: Optional[List[int]] = None, resultados_tempo_real: Optional[Dict] = None) -> pd.DataFrame:
    ...
    if not todas_linhas:
        ...
        print(f"{yellow('[!]')} Nenhum dado para consolidar")
        return pd.DataFrame()
    ...
    try:
        ...
        if safe_save_csv(df_consolidado, file):
            print_success(f"Consolidado: {file}", len(df_consolidado))
...
def coletar_configuracao() -> Optional[Dict]:
    ...
    print(f"\n{cyan(bold('Mini Research v4.8 - Configuração'))}\n")
...
def main():
    ...
    print(f"\n{cyan(bold('Mini Research v5.1 - Coletor de Dados Multi-Fonte'))}\n")
