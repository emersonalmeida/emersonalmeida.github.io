...
# -*- coding: utf-8 -*-
"""
Mini Research v4.8 - Coletor de Dados Multi-Fonte com Análises e Dashboard
Versão inteligente com coleta múltipla e ordenação avançada
        ...
        try:
            ...
            try:
                ...
                if r and "rising" in r and r["rising"] is not None:
                            ...
                            print_progress(f"Rising relacionados ({tipo_nome}, {periodo_nome}): {len(df)} itens")
                    for i, row in enumerate(df.itertuples(), 1):
                                item = {
                                    "tipo": "rising", 
                                    "fonte": tipo_nome, 
            ...
            try:
                ...
                if not df_time.empty:
                            print_progress(f"Interesse ao longo do tempo ({tipo_nome}, {periodo_nome}): {len(df_time)} pontos")
...
def run_youtube(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> Dict:
    ...
    print_progress(f"Total de vídeos únicos encontrados: {len(videos)}")
...
def run_stores(termo: str, config: Dict, output_dir: str, resultados_tempo_real: Dict) -> Dict:
    ...
    for loja_idx, loja_id in enumerate(lojas_order, 1):
        ...
        if not df_apple.empty:
                ...
                print_progress(f"App Store: {len(df_apple)} apps encontrados (solicitados: {n_apps_efetivo})")
...
def coletar_config_fonte(fonte_id: int, config: Dict) -> Dict:
    ...
    if fonte_id == 1:  # Google Suggest
        ...
        limit = safe_int_input("  Resultados [15]: ", 15, min_val=1, max_val=10000)  # v4.8: aumentar limite máximo (padrão 15)
        # v4.8: Se número muito grande, usar máximo possível (será tratado nas funções de coleta)
        fonte_config = {
        ...
        limite = safe_int_input("  Resultados [20]: ", 20, min_val=1, max_val=1000)  # v4.8: aumentar limite
        # v4.8: Coletar resultados individuais por buscador (padrão 20 por buscador)
        fonte_config = {
        ...
        if coletar_comentarios == "s":
            limite_comentarios = safe_int_input("  Comentários por vídeo [20]: ", 20, min_val=1, max_val=1000)  # v4.8: aumentar limite
        ...
        fonte_config = {
            ...
            "limite_comentarios_por_ordenacao": limite_comentarios,  # v4.8: limite por ordenação
        ...
        if coletar_reviews == "s":
            max_reviews = safe_int_input("  Reviews por app [20]: ", 20, min_val=1, max_val=10000)  # v4.8: aumentar limite
        ...
        fonte_config = {
            ...
            "max_reviews_por_ordenacao": max_reviews,  # v4.8: reviews por ordenação
            ...
            "review_orders_order": review_order_selected if coletar_reviews == "s" else []  # v4.8: manter ordem
        }
...
def coletar_configuracao() -> Optional[Dict]:
    ...
    # 01. TERMOS DE BUSCA (com sanitização e cores padronizadas - v4.8)
    ...
    # 02. REGIÃO (com mapeamento automático de idioma - v4.8)
    ...
    # Mapear idiomas automaticamente por região - v4.8
    ...
    # Aplicar parâmetros globais com idioma automático - v4.8
    for fonte_config in ["suggest", "trends", "serp", "youtube", "stores"]:
        if fonte_config in config:
            ...
            # v4.8: Usar idioma mapeado automaticamente
