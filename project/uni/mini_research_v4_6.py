...
# -*- coding: utf-8 -*-
"""
Mini Research v4.6 - Coletor de Dados Multi-Fonte com Análises e Dashboard
Versão inteligente com coleta máxima de dados e metadados completos
            ...
            try:
                ...
                for vid in stats_response.get("items", []):
                    videos_detalhes[vid["id"]] = {
                        "viewCount": int(vid.get("statistics", {}).get("viewCount", 0)),
                        "likeCount": int(vid.get("statistics", {}).get("likeCount", 0)),
                        "commentCount": int(vid.get("statistics", {}).get("commentCount", 0)),
                        "duration": vid.get("contentDetails", {}).get("duration", ""),
                        "thumbnails": vid.get("snippet", {}).get("thumbnails", {}),
                        "channelId": vid.get("snippet", {}).get("channelId", ""),
                        "channelUrl": f"https://www.youtube.com/channel/{vid.get('snippet', {}).get('channelId', '')}",
                        "tags": vid.get("snippet", {}).get("tags", []),
                        "categoryId": vid.get("snippet", {}).get("categoryId", "")
                    }
            except Exception as e:
...
def apple_df(results):
    ...
    for r in results:
        rows.append({
            ...
            "supportedDevices": r.get("supportedDevices", []),
...
def fetch_google(term, lang="pt", country="br", n=20):
    ...
    for r in res[:n]:
        rows.append({
            ...
            "ratings_count": r.get("ratings", 0),  # Total de avaliações
            "reviews_count": r.get("reviews", 0),  # Total de reviews
            ...
            while len(out) < max_reviews:
                ...
                for review in batch:
                    expanded_review = {
                        ...
                        "at": review.get("at", ""),
                        "replyContent": review.get("replyContent", ""),  # Resposta do desenvolvedor
                        "repliedAt": review.get("repliedAt", ""),  # Data da resposta
...
def normalizar_dados_fonte(fonte_id: int, dados: Any, termo: str) -> List[Dict]:
    ...
    if fonte_id == 1 and isinstance(dados, list):  # Google Suggest
        ...
        # Processar vídeos com TODOS os metadados - v4.6
        ...
        # Processar comentários com TODOS os metadados - v4.6
    ...
    # (caso não estejam no dict retornado por run_youtube)
        ...
        for loja, df in dados.items():
            if isinstance(df, pd.DataFrame) and not df.empty:
                # Processar apps com TODOS os metadados - v4.6
            ...
            for item in resultados_tempo_real["stores"]:
                if isinstance(item, dict) and item.get("tipo") == "review":
                    ...
                    todas_linhas.append({
                        ...
                        "replyContent": review_data.get("replyContent", ""),  # Resposta do dev
                        "repliedAt": review_data.get("repliedAt", ""),  # Data da resposta
                        "data_coleta": data_coleta
                    })
