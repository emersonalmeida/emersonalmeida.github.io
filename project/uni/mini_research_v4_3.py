...
# -*- coding: utf-8 -*-
"""
Mini Research v4.3 - Coletor de Dados Multi-Fonte com Análises e Dashboard
Versão com interface simplificada e focada no conteúdo

Melhorias v4.3:
- Interface limpa sem símbolos excessivos
- Remoção de elementos redundantes e repetitivos
- Consolidação de opções "Todos"
- Headers simplificados
- Menos elementos de layout, mais foco no conteúdo
- Remoção de confirmações duplicadas
- Menu mais direto e objetivo
...
def print_config_summary(config):
    """Exibe resumo simplificado - v4.3"""
    print("\nResumo da configuração:")
    print(f"  Termo: {config.get('termo', 'N/A')}")
    print(f"  Região: {', '.join(config.get('regions', []))}")
    print(f"  Fontes: {len(config.get('fontes', []))} selecionada(s)")
...
def exibir_dashboard(resultados, termo, output_dir, stats, insights, fontes_ordenadas=None):
    """Exibe dashboard completo simplificado - v4.3"""
    print(f"\nDashboard - {termo}\n")
    
    # Resumo geral
    print("Resumo geral:")
    if stats and "geral" in stats:
        print(f"  Total de itens: {stats['geral']['total_itens']}")
        print(f"  Fontes de dados: {stats['geral']['fontes_unicas']}")
    ...
    if stats and fontes_ordenadas:
        ...
        for fonte_id in fontes_ordenadas:
            ...
            if fonte_stats:
                print(f"\n  {fonte_nome}:")
                print(f"    Total: {fonte_stats['total']} itens")
        ...
        for fonte, dados in stats.items():
            if fonte != "geral":
                print(f"\n  {fonte}:")
                print(f"    Total: {dados['total']} itens")
                for tipo, count in dados["tipos"].items():
                    print(f"    - {tipo}: {count}")
    ...
    if HAS_MATPLOTLIB:
        print("  ✓ Gráficos")
    print("  ✓ Arquivos individuais por fonte")
...
def coletar_configuracao():
    """Coleta configuração simplificada v4.3"""
    print("\nMini Research v4.3 - Configuração\n")
    ...
    fontes_input = input("> [t]: ").strip() or "t"
    if fontes_input.lower() in ["t", "todos"]:
    ...
    for fonte_id in fontes_ordenadas:
        if fonte_id == 1:  # Google Suggest
            ...
            if clients_input.lower() in ["t", "todos"]:
            ...
            if sources_input.lower() in ["t", "todos"]:
            ...
            opcoes_input = input("  > [t]: ").strip() or "t"
            if opcoes_input.lower() in ["t", "todos"]:
                opcoes_selected = list(range(1, 5))
            else:
                opcoes_selected = parse_numeric_input(opcoes_input, SUGGEST_OPCOES, default=1)
            
            limit_input = input("  Resultados [15]: ").strip() or "15"
            ...
            topn_input = input("  Resultados [20]: ").strip() or "20"
            ...
            limite_videos_input = input("  Vídeos [10]: ").strip() or "10"
            ...
            if comment_order_input.lower() in ["t", "todos"]:
            ...
            if lojas_input.lower() in ["t", "todos"]:
                lojas_selected = [1, 2]
            else:
                lojas_selected = parse_numeric_input(lojas_input, lojas_options, default=1)
            
            n_apps_input = input("  Aplicativos [10]: ").strip() or "10"
            n_apps = int(n_apps_input) if n_apps_input.isdigit() else 10
            
            max_reviews_input = input("  Reviews [100]: ").strip() or "100"
            ...
            if review_order_input.lower() in ["t", "todos"]:
    ...
    # DELAY
    delay_input = input("\nDelay entre requisições [0]: ").strip() or "0"
...
def main():
    print("\nMini Research v4.3 - Coletor de Dados Multi-Fonte\n")
