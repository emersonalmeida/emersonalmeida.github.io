...
def def print_config_summary(config):
...
def consolidar_dados(resultados, termo, output_dir):
    """Consolida todos os dados coletados em um único DataFrame"""
...
def gerar_estatisticas(df_consolidado, termo, output_dir):
    ...
    print(cyan("\n📊 ESTATÍSTICAS POR FONTE"))
    print("-" * 70)
    for fonte in df_consolidado["fonte"].unique():
...
def gerar_insights(resultados, termo, output_dir):
    ...
    for k, v in resultados.items():
        if v and k != "consolidado":
            if isinstance(v, dict):
                count = sum(len(item) if isinstance(item, (list, pd.DataFrame)) else 1 for item in v.values() if item)
    ...
    if stats and fontes_ordenadas:
        for fonte_id in fontes_ordenadas:
            ...
            for stat_fonte, dados in stats.items():
                if stat_fonte != "geral" and fonte_key in stat_fonte.lower():
                    fonte_stats = dados
                    break
...
def main():
    ...
    # 1. Consolidar todos os dados
    df_consolidado = consolidar_dados(resultados, termo, output_dir)
    resultados["consolidado"] = df_consolidado
    
    # 2. Gerar estatísticas
    stats = gerar_estatisticas(df_consolidado, termo, output_dir)
    ...
    for fonte_id in fontes_ordenadas:
        ...
        if fonte_key in resultados and resultados[fonte_key]:
            ...
            if isinstance(dados, dict):
                count = sum(len(v) if isinstance(v, (list, pd.DataFrame)) else 1 for v in dados.values() if v)
